...
import datetime
import pandas as pd
from eval_utils.eval_weather_data_util import load_bj_grid_meo_data, load_ld_grid_meo_data
...
def meo_data_preprocess(city="bj"):
    ...
    meo_stations_merged.to_csv(
        eval_processed_data_dir+"%s_meo_data.csv" % (city))
    ...
    # describe = pd.read_csv("./eval_utils/%s_meo_describe.csv" % (city))
    describe.to_csv(eval_processed_data_dir + "%s_meo_describe.csv" % (city))

    # describe = pd.read_csv(test_data_dir+"%s_meo_describe.csv" % (city))
    ...
    # df_norm = (meo_stations_merged - c[1:]) / d[1:]
    df_norm = (meo_stations_merged -
               describe.loc['mean']) / describe.loc['std']
    df_norm.to_csv(eval_processed_data_dir+"%s_meo_norm_data.csv" % (city))
