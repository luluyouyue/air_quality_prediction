...
def generator_result(model_preds, city='bj', pre_days=5):
    ...
    X_meo_list = ["temperature", "pressure",
                  "humidity", "direction", "speed/kph"]

    # ld_station_list = ['BL0','CD1','CD9','GN0','GN3','GR4','GR9','HV1','KF1','LW2','MY7','ST5','TH4']
    ld_station_list = ['CD1', 'BL0', 'GR4', 'MY7', 'HV1',
                       'GN3', 'GR9', 'LW2', 'GN0', 'KF1', 'CD9', 'ST5', 'TH4']
    ...
    # Notice to keep it consistent with submit.to_csv
    if pre_days == 5:
        submit = pd.read_csv(KddConfig.submission_pre5day_file_path)
    elif pre_days == 2:
        submit = pd.read_csv(KddConfig.submission_pre2day_file_path)
    else:
        print("add file to config.py")
        submit = pd.read_csv(KddConfig.submission_file_path)
    ...
    if city == 'ld':
        # translate  downloaded  airQuality_id  to csv
        list = []
        name_0 = ['BL0_PM10', 'BL0_PM2.5', 'CD1_PM10', 'CD1_PM2.5', 'CD9_PM10', 'CD9_PM2.5', 'GN0_PM10', 'GN0_PM2.5', 'GN3_PM10', 'GN3_PM2.5', 'GR4_PM10', 'GR4_PM2.5', 'GR9_PM10',
                  'GR9_PM2.5', 'HV1_PM10', 'HV1_PM2.5', 'KF1_PM10', 'KF1_PM2.5', 'LW2_PM10', 'LW2_PM2.5', 'MY7_PM10', 'MY7_PM2.5', 'ST5_PM10', 'ST5_PM2.5', 'TH4_PM10', 'TH4_PM2.5']
        aq_shape = np.array(model_preds).shape
        for i in range(aq_shape[0]):
...
#        print np.array(new_predict).shape  # (1, 624)
    ...
    submit.to_csv(KddConfig.submission_file_dir+'pre_days_'+str(pre_days) +
