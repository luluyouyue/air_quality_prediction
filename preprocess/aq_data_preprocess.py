...
def aq_data_preprocess(city="bj"):
    ...
    # print(type(aq_data))
    # print(type(stations))
    # print(type(aq_stations))
    # print(type(aq_stations_merged))

    print('aq_stations_merged')
    print(aq_stations_merged.columns)
    print(aq_stations_merged.iloc[0:3,:])
    ...
    print('hours_should '+str(hours_should))
    ...
    for hour in missing_hours_str : 
        ...
        if all_steps > 5 :
            ...
            # addedHour = pd.Series({key:np.nan for key in df_merged.columns})
            # addedHour =  for_row + (for_step/all_steps) * delata_values
            df_merged.loc[hour] = for_row + (for_step/all_steps) * delata_values
    print('after time absence handle')
    print(df_merged.columns)
    print(df_merged.shape)
