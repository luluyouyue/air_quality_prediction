...
from preprocess.aq_data_util import load_bj_aq_data, load_ld_aq_data
...
def aq_data_preprocess(city="bj"):
    ...
    if city == "bj":
        ...
        aq_station_locations = pd.read_csv(
            KddConfig.ld_aq_locations)
    ...
    df_merged.to_csv(processed_data_dir + "%s_aq_data.csv" % (city))
    print(df_merged.shape)

    # ### 4. 数据归一化

    describe = df_merged.describe()
    describe.to_csv(processed_data_dir + "%s_aq_describe.csv" % (city))

    df_norm = (df_merged - describe.loc['mean']) / describe.loc['std']
    df_norm.to_csv(processed_data_dir + "%s_aq_norm_data.csv" % (city))
