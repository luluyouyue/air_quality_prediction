...
path_to_bj_aq = KddConfig.path_to_new_bj_aq
# "../KDD_CUP_2018/test_data/London/aq/"
path_to_ld_aq = KddConfig.path_to_new_ld_aq
...
def load_bj_aq_data():
    ...
    # print(bj_aq_df.columns) out: [u'stationId', u'utc_time', u'PM2.5', u'PM10', u'NO2', u'CO', u'O3',
    #        u'SO2']
    # print(bj_aq_df.shape)  out: (421225, 8)

    # 按时间升序重新排列数据
    bj_aq_df.sort_index(inplace=True)   # sort the rows
    # print('sorted pd index:')
    # print(bj_aq_df.columns)
    bj_aq_df.drop_duplicates(subset=None, keep='first', inplace=True)
    # print('after drop:')
    # print(bj_aq_df.shape) out: (408100, 8)

    bj_aq_dataset, stations, bj_aq_stations, bj_aq_stations_merged = load_city_aq_data(
        bj_aq_df)
...
def load_city_aq_data(aq_df):
    ...
    for station in stations:
        ...
        aq_station.drop_duplicates('utc_time', keep='last', inplace=True)
        aq_station["time"] = pd.to_datetime(aq_dataset['utc_time'])
        aq_station.set_index("time", inplace=True)
        aq_station.drop("utc_time", axis=1, inplace=True)

        # rename
        original_names = aq_station.columns.values.tolist()
        names_dict = {original_name: station+"_" +
        ...
        df_list.append(aq_station)
        print(aq_station.shape)  # ( , 3)
    print('aq_station number:', len(aq_stations), len(df_list))
    # for station in stations :
    ...
    aq_stations_merged = pd.concat(df_list, axis=1)
    # add a column of (0,1,2,3,...) to count
    length = aq_stations_merged.shape[0]
    order = range(length)
    aq_stations_merged['order'] = pd.Series(
        order, index=aq_stations_merged.index)
