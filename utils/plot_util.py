...
#     KDD_CUP_2018 = bj_aq_stations[station].loc[minimum:maxmimum]
#     for column in KDD_CUP_2018.columns :
#         if feature in column :
#             features = KDD_CUP_2018[column]
#             print(type(features))
#             print(features)
...
def plot_forecast_and_actual_example(test_x, test_y, final_preds, features, index, feature_index):
    ...
    ax.plot(range(input_seq_len), x, label="test x KDD_CUP_2018");
    ax.plot(range(input_seq_len, input_seq_len + output_seq_len), y_t, label="test y KDD_CUP_2018")
    ax.plot(range(input_seq_len, input_seq_len + output_seq_len), y_p, label="prediction y KDD_CUP_2018")
    ax.legend();
